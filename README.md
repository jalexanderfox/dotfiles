...
## NEW
Checkout the new [Docker](#user-content-docker) section, several scripts that can be useful for working with docker on a mac.
...
there is a respective alias:
```bash
docker_debugContainer
docker_profile
docker_setOSXHosts
```

Currently there are aliases for:
* add/remove hosts map to /etc/hosts (see docker/conf/hostsToMap for configuration)
```bash
docker_setOSXHosts
docker_unsetOSXHosts
```
...
```bash
docker_createMachines [name]
```

* create basic machines (office/vpn)
```bash
docker_createMachines
```

* debug a container
```bash
docker_debugContainer [container_name]
```

* fresh IPs (if any of your IPs change docker-machine or host machine)
```bash
docker_freshIPs
docker_freshIPsWithProxy
```
...
* list all docker custom scripts
```bash
docker_ls
```
...
```bash
docker_restartMachine
```

* show the docker-machine profile
```bash
docker_profile
```
...
In addition to these scripts, following functions are available (only if you have installed/setup the [dotfiles](#user-content-installation) in this repository):
